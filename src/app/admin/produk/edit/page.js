...
export default function EditProduk() {
  ...
  // Dummy data, nanti diganti fetch dari database
  const [nama, setNama] = useState("Classic Arabica");
  const [harga, setHarga] = useState("30000");
  const [linkShopee, setLinkShopee] = useState("https://shopee.co.id/classic-arabica");
  const [deskripsi, setDeskripsi] = useState("Semiwash Process\nProses olah giling basah (semi wash process), berlangsung dengan cara mengupas biji kopi setengah kering yang masih berbentuk gabah basah, lalu setelahnya dijemur dibawah sinar matahari sampai tingkat kadar air ideal.");
  const [gambar, setGambar] = useState(null);

  const handleSubmit = (e) => {
    e.preventDefault();
    // Integrasi database di sini
    alert('Produk berhasil diupdate (dummy)');
